...
try {
  ...
  console.log(bad("✗ Printful FAILED:"), e.message);
}
...
try {
  ...
  if (r.ok) {
    ...
    for (const s of j)
      console.log("   id=", s.id, " title=", dim(s.title), " sales_channel=", dim(s.sales_channel));
  } else {
    console.log(bad("✗ Printify auth FAILED"), r.status, dim(JSON.stringify(j).slice(0, 200)));
  }
} catch (e) {
  console.log(bad("✗ Printify FAILED:"), e.message);
}
